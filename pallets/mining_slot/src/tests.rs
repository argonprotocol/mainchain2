...
use argon_primitives::{
	...
	AuthorityProvider, BlockVote, MerkleProof,
};
use frame_support::{
...
fn it_holds_ownership_tokens_for_a_slot() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			MiningSlots::bid(RuntimeOrigin::signed(2), 0, RewardDestination::Owner, 1.into()),
		...
		assert_err!(
			MiningSlots::bid(RuntimeOrigin::signed(1), 0, RewardDestination::Owner, 1.into()),
...
fn it_will_order_bids() {
	...
	new_test_ext().execute_with(|| {
		...
		// Bids must be increments of 1 argonot
		assert_err!(
			MiningSlots::bid(
				RuntimeOrigin::signed(1),
				100_000u32.into(),
		...
		assert_eq!(
			...
			5_000_000,
			"doesn't refund anything til the end"
		);
		assert_eq!(Balances::free_balance(1), 2_100_000, "bid 1 is still held");
		System::assert_has_event(
			Event::SlotBidderAdded { account_id: 3, bid_amount: 2_000_000, index: 1 }.into(),
		);
		System::assert_last_event(
			Event::SlotBidderOut { account_id: 1, bid_amount: 1_000_000 }.into(),
		);
		...
		assert_eq!(Balances::free_balance(bid_pool_account_id), 7_000_000);

		let first_bid = HistoricalBidsPerSlot::<Test>::get().into_inner()[0].clone();
		assert_eq!(first_bid.bids_count, 4);
		assert_eq!(first_bid.bid_amount_min, 1_000_000);
		assert_eq!(first_bid.bid_amount_max, 3_000_000);
		assert_eq!(first_bid.bid_amount_sum, 8_000_000);

		// compare to the last event record
		System::assert_last_event(
			Event::SlotBidderOut { account_id: 3, bid_amount: 2_000_000 }.into(),
		);

		System::assert_has_event(
			Event::SlotBidderAdded { account_id: 1, bid_amount: 3_000_000, index: 0 }.into(),
		);
		assert_eq!(Ownership::free_balance(3), 100_000);
...
fn handles_a_max_of_bids_per_block() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			MiningSlots::bid(RuntimeOrigin::signed(3), 0, RewardDestination::Owner, 3.into()),
